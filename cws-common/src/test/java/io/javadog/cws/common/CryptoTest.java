...
//        final CWSKey key = crypto.generateKey(KeyAlgorithm.RSA2048);
//        final byte[] message = "Message to Sign".getBytes(settings.getCharset());
//        final String signed = crypto.sign(key, message);
//        final boolean verified = crypto.verify(key, message, signed);
//
...
//    }
    ...
    public void testArmoringPrivateKey() {
        ...
        final CWSKey cryptoKeys = crypto.generateKey(KeyAlgorithm.RSA2048);
        final CWSKey keyPair = crypto.generateKey(KeyAlgorithm.RSA2048);
...
//        final CWSKey keyPair = crypto.generateKey(KeyAlgorithm.RSA2048);
//
//        final String armoredKey = crypto.armoringSecretKey(keyPair, key);
...
//        final CWSKey key = crypto.generateKey(KeyAlgorithm.AES128, dataSalt);
...
//        final SecretKey circleKey = crypto.extractCircleKey(key.getAlgorithm(), keyPair, armoredCircleKey);
