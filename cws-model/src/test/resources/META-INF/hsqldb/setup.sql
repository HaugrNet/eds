...
CREATE TABLE settings (
  ...
  /* Unique Constraints */
  CONSTRAINT setting_unique_external_id     UNIQUE (external_id),
  CONSTRAINT setting_unique_name            UNIQUE (name),

  /* Not Null Constraints */
  CONSTRAINT setting_notnull_id             CHECK (id IS NOT NULL),
  CONSTRAINT setting_notnull_external_id    CHECK (external_id IS NOT NULL),
...
-- the limited Key lengths, of the Unlimited Key Strength library has been
...
CREATE TABLE keys (
  ...
  /* Unique Constraints */
  CONSTRAINT key_unique_external_id         UNIQUE (external_id),

  /* Not Null Constraints */
  CONSTRAINT key_notnull_id                 CHECK (id IS NOT NULL),
  CONSTRAINT key_notnull_external_id        CHECK (external_id IS NOT NULL),
  ...
  CONSTRAINT key_notnull_initial_vector     CHECK (initial_vector IS NOT NULL),
...
CREATE TABLE types (
  ...
  /* Unique Constraints */
  CONSTRAINT type_unique_external_id        UNIQUE (external_id),
  CONSTRAINT type_unique_name               UNIQUE (type_name),

  /* Not Null Constraints */
  CONSTRAINT type_notnull_id                CHECK (id IS NOT NULL),
  CONSTRAINT type_notnull_external_id       CHECK (external_id IS NOT NULL),
...
CREATE TABLE objectdata (
  ...
  CONSTRAINT storage_object_fk              FOREIGN KEY (object_id) REFERENCES objects (id),
  CONSTRAINT storage_key_fk                 FOREIGN KEY (key_id) REFERENCES keys (id),

  /* Not Null Constraints */
  CONSTRAINT storage_notnull_id             CHECK (id IS NOT NULL),
  CONSTRAINT storage_notnull_object_id      CHECK (object_id IS NOT NULL),
  CONSTRAINT storage_notnull_key_id         CHECK (key_id IS NOT NULL),
  CONSTRAINT storage_notnull_data           CHECK (encrypted_data IS NOT NULL )
)
